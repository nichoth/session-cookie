 ...
 */
export function getSession (context:HandlerContext) {
...
function getCookies (ev:HandlerEvent):string[]|undefined {
...
//     const signature = cookies[cookieName].substring(
//         0,
...
async function sessionWrapper (
    ...
    if (incomingCookies) {
        cookieMap = parseCookie(incomingCookies[0])
    }
    ...
    if (cookieMap! && cookieMap[cookieName]) {
        ...
        // parsed from JSON.
        let data = cookieMap[cookieName].substring(SIGNATURE_DIGEST_LENGTH)
        data = Buffer.from(data, 'base64').toString('utf-8')

        // If signature matches, parse data from JSON and put into the
        // `clientContext.session` object.
        if (verify([getSecretKey()], data, signature)) {
            data = JSON.parse(data)

            // Update in place to preserve `session` ref.
    ...
    // `context` contains parsed session data that can be read and edited
    //   directly from the handler.

    //
    // [3] Process response out of the handler to automatically append session
    //     data as a signed cookie.
    //
